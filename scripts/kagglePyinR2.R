...
for (i in 1:length(testData[,1])){
  fare <- testData[i,]$fare
  if (is.na(testData[i,]$fare) || testData[i,]$fare < 10 )
    fare = 10
  else if(testData[i,]$fare <20)
    fare = 20
  else if (testData[i,]$fare <30)
    fare = 30
  else
    fare=512
  sex2 <- testData[i,]$sex
  passengerClass <- testData[i,]$pclass
  s <- dicDataFrame$survivalRatio[dicDataFrame$sex==sex2 & dicDataFrame$cabin==passengerClass & dicDataFrame$price==fare]
  #ss <- subset (dicDataFrame, dicDataFrame$sex==sex2 & dicDataFrame$cabin==passengerClass & dicDataFrame$price==fare, select=c("survivalRatio") )
  cat(sex2,"|",passengerClass,"|",fare,"--->", s,"\n")
...
t <- data.frame("survived"=testSurvived, testData)
